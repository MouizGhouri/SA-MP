...
{
	if(dm[playerid] == 1) return GameTextForPlayer(playerid, "~g~You are already in~n~~w~deagle deathmatch", 4500, 3);
	DDM(playerid);
	return 1;
}
...
{
	if(dm[playerid] == 2) return GameTextForPlayer(playerid, "~g~You are already in~n~~w~sniper deathmatch", 4500, 3);
	SDM(playerid);
	return 1;
}
...
{
	if(dm[playerid] == 3) return GameTextForPlayer(playerid, "~g~You are already in~n~~w~sawn off deathmatch", 4500,3);
	SOSDM(playerid);
	return 1;
}
